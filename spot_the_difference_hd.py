...
------------------------------------------------
Desktop application built with Tkinter, OpenCV and OOP.

Main features matched to the assignment brief:
    * Loads JPG, JPEG, PNG and BMP images from disk.
    * Displays original image on the left and modified image on the right.
    * Uses OpenCV to clone the image and create exactly 5 random differences.
    * Guarantees that all generated difference regions are non-overlapping.
    * Uses 4 alteration classes to demonstrate inheritance and polymorphism.
    * Player clicks only the modified image to find differences.
    * Correct clicks draw red circles on both images.
    * Wrong clicks increase the mistake counter.
    * After 3 mistakes, the round is locked and the user must load a new image.
    * Reveal button shows all unfound differences with blue circles on both images.
...
class Alteration(ABC):
    ...
    Subclasses override apply(). The game stores them in one list and calls
    apply() polymorphically, which demonstrates inheritance and polymorphism.
    """
...
class ColourShiftAlteration(Alteration):
    ...
    def apply(self, image: np.ndarray, region: DifferenceRegion, rng: random.Random) -> None:
        ...
        # Small but visible changes. All arithmetic is done safely using int arrays.
...
class BlurAlteration(Alteration):
    """Applies a local Gaussian blur to soften one region."""
...
class BrightnessAlteration(Alteration):
    """Brightens or darkens a selected region."""
...
class ShapeOverlayAlteration(Alteration):
    """Draws a small semi-transparent shape inside the selected region."""
    ...
    def apply(self, image: np.ndarray, region: DifferenceRegion, rng: random.Random) -> None:
        ...
        # Use a colour related to the local mean so the shape is visible but not too harsh.
        roi = self._roi(image, region)
...
class DifferenceGenerator:
    """Creates non-overlapping difference regions and applies random alterations."""
    ...
    def alteration_names(self) -> List[str]:
        """Names of available alteration strategies."""
...
class GameRound:
    """Maintains all state for the current image/game round."""
    ...
    def remaining(self) -> int:
        """Number of differences still hidden in this round."""
    ...
    def process_click(self, x: int, y: int) -> Tuple[str, Optional[DifferenceRegion]]:
        """
        Handle one click in original image coordinates.

        Returns:
            ("found", region)     when the click matches a hidden difference.
            ("mistake", None)    when the click is incorrect.
            ("locked", None)     when no more guesses are allowed.
            ("completed", None)  when all differences have been found.
        """
    ...
    def reveal_unfound(self) -> None:
        """Reveal all remaining hidden differences and lock the current round."""
        for region in self.regions:
            if not region.found:
    ...
    def render_original_with_marks(self) -> np.ndarray:
        """Return original image copy with red/blue feedback circles."""
        return self._draw_marks(self.original_bgr.copy())

    def render_modified_with_marks(self) -> np.ndarray:
        """Return modified image copy with red/blue feedback circles."""
        return self._draw_marks(self.modified_bgr.copy())

    def _draw_marks(self, image: np.ndarray) -> np.ndarray:
        """Draw red circles for found regions and blue circles for revealed regions."""
        for region in self.regions:
            if region.found:
...
class ImagePanel(tk.Frame):
    """Canvas panel that displays a BGR OpenCV image while preserving aspect ratio."""
    ...
    def show_image(self, image_bgr: np.ndarray) -> None:
        """Display image_bgr in the canvas using OpenCV resizing and PIL conversion."""
...
class SpotDifferenceApp(tk.Tk):
    ...
    def load_image(self) -> None:
        """Open a file dialog and start a completely new round."""
    ...
    def handle_modified_click(self, event: tk.Event) -> None:
        """Validate a player click on the modified image."""
    ...
    def reveal_differences(self) -> None:
        """Reveal all currently unfound differences in blue."""
        if self.round is None:
            self._update_status("Please load an image first.")
            return
    ...
    def _refresh_images(self) -> None:
        """Redraw both panels and update counters."""
        if self.round is None:
            return
    ...
    def _update_status(self, message: str) -> None:
        """Update the lower-right status text."""
        self.message_var.set(message)

    def show_help(self) -> None:
        """Display simple game instructions."""
        messagebox.showinfo(
            "How to Play",
    ...
    def show_oop_summary(self) -> None:
        """Explain the OOP structure for markers and group presentation."""
        messagebox.showinfo(
            "OOP Design Summary",
            "Classes used:\n"
...
def check_dependencies() -> None:
    """Give a clear message if the user runs the program without required packages."""
    missing = []
    try:
    ...
    if missing:
        ...
        print("Install them using: pip install opencv-python pillow numpy")
        sys.exit(1)
